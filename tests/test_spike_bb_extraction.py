...
from tidalsim.bb_extraction.spike import *
...
core   0: 0x0000000080001a72 (0x00000613) li      a2, 0""".split('\n')
        ...
        assert result == [
            SpikeTraceEntry(0x8000_0104, 0x30529073, "csrw"),
            SpikeTraceEntry(0x8000_0108, 0x169010ef, "jal"),
            SpikeTraceEntry(0x8000_1a70, 0x00001141, "c.addi"),
            SpikeTraceEntry(0x8000_1a72, 0x00000613, "li"),
        ]

    def test_single_large_block_bbs(self) -> None:
        # A single large block
        result = spike_trace_to_bbs(iter([
            SpikeTraceEntry(0x4, 0x0, "c.addi"),
            SpikeTraceEntry(0x6, 0x0, "c.addi"),
            SpikeTraceEntry(0x8, 0x0, "li"),
            SpikeTraceEntry(0xc, 0x0, "jal"),
        ])).pc_to_bb_id
    ...
    def test_two_blocks_bbs(self) -> None:
        # Two blocks
        result = spike_trace_to_bbs(iter([
            SpikeTraceEntry(0x4, 0x0, "li"),
            SpikeTraceEntry(0x8, 0x0, "li"),
            SpikeTraceEntry(0xc, 0x0, "jal"),
            SpikeTraceEntry(0x20, 0x0, "add"),
            SpikeTraceEntry(0x24, 0x0, "add"),
        ])).pc_to_bb_id
        assert result == IntervalTree([
            Interval(0x4, 0xc+1, 0),
    ...
    def test_block_splitting(self) -> None:
        ...
        result = spike_trace_to_bbs(iter([
            SpikeTraceEntry(0x4, 0x0, "li"), # Initially a single block from 0x4-0xc
            SpikeTraceEntry(0x8, 0x0, "li"),
            SpikeTraceEntry(0xc, 0x0, "jal"),
            SpikeTraceEntry(0x20, 0x0, "add"), # The second block from 0x20-0x28
            SpikeTraceEntry(0x24, 0x0, "add"),
            SpikeTraceEntry(0x28, 0x0, "beq"),
            SpikeTraceEntry(0x8, 0x0, "li"), # The first block is now split into 2
            SpikeTraceEntry(0xc, 0x0, "jal"),
            SpikeTraceEntry(0x20, 0x0, "add"),
            SpikeTraceEntry(0x24, 0x0, "add"),
            SpikeTraceEntry(0x28, 0x0, "beq"),
    ...
    def test_single_inst_bb(self) -> None:
        ...
        result = spike_trace_to_bbs(iter([
            ...
            SpikeTraceEntry(0x8, 0x0, "li"),
            SpikeTraceEntry(0xc, 0x0, "jal"),
            SpikeTraceEntry(0x20, 0x0, "jal"), # This instruction is one basic block alone
            SpikeTraceEntry(0x30, 0x0, "add"),
            SpikeTraceEntry(0x34, 0x0, "sub"),
        ])).pc_to_bb_id
        assert result == IntervalTree([
            Interval(0x4, 0xc+1, 0),
    ...
    def test_immediate_jumps(self) -> None:
        ...
        result = spike_trace_to_bbs(iter([
            SpikeTraceEntry(0x4, 0x0, "li"),
            SpikeTraceEntry(0x8, 0x0, "li"),
            SpikeTraceEntry(0xc, 0x0, "jal"), # Jumps to PC + 4, but technically creates a new basic block
            SpikeTraceEntry(0x10, 0x0, "add"),
            SpikeTraceEntry(0x14, 0x0, "add"),
        ])).pc_to_bb_id
        assert result == IntervalTree([
            Interval(0x4, 0xc+1, 0),
    ...
    def test_uncaught_control_insts(self) -> None:
        with pytest.raises(RuntimeError):
            spike_trace_to_bbs(iter([
                SpikeTraceEntry(0x4, 0x0, "li"),
                SpikeTraceEntry(0x8, 0x0, "li"),
                SpikeTraceEntry(0xc, 0x0, "???"),  # We saw a PC break, but not a known control instruction
                SpikeTraceEntry(0x14, 0x0, "add"),
            ]))
